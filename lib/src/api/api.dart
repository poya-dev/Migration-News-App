...
class ApiService {
  ...
  static Future<User> signInWithGoogle(String idToken) async {
    try {
      final response = await client.get(Uri.parse('$baseUrl/auth/google'),
          headers: {
      ...
      throw Exception('---inside api Something went wrong ---> $e');
    }
  }

  static Future<User> signInWithFacebook(String accessToken) async {
    try {
      final response = await client.post(Uri.parse('$baseUrl/auth/facebook'),
          headers: {
